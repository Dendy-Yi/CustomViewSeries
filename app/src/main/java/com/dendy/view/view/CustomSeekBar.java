...
public class CustomSeekBar extends View {
    ...
    public void initData(ArrayList<String> section){
        if(section != null){
            ...
            String[] str = new String[]{"0", "10", "15","30","60","90"};
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawLine(bitMapHeight, height / 3, width - bitMapHeight - spot_on.getWidth() / 2, height / 3, mPaint);
        int section = 0;
        while(section < section_title.size()){
            if(section < cur_sections) {
                mPaint.setColor(colors[0]);
                canvas.drawLine(thumb.getWidth()/2 + section * perWidth + (section+1) * spot_on.getWidth(),height / 3,
                        thumb.getWidth()/2 + section * perWidth + (section+1) * spot_on.getWidth() + perWidth,height / 3,mPaint);
                canvas.drawBitmap(spot_on, thumb.getWidth()/2 + section * perWidth + section * spot_on.getWidth(),height / 3 - spot_on.getHeight()/2,mPaint);
            }else{
                mPaint.setAlpha(255);
                if(section == section_title.size()-1){
                    canvas.drawBitmap(spot,  width - spot_on.getWidth() - bitMapHeight/2, height / 3 - spot.getHeight() / 2, mPaint);
                }else {
                    canvas.drawBitmap(spot, thumb.getWidth()/2 + section * perWidth + section * spot_on.getWidth(), height / 3 - spot.getHeight() / 2, mPaint);
                }
            }

            if(section == section_title.size()-1) {
                canvas.drawText(section_title.get(section), width - spot_on.getWidth()- bitMapHeight/4 - textSize / 2, height / 3 + textMove, mTextPaint);
            }else{
                canvas.drawText(section_title.get(section), thumb.getWidth()/2 + section * perWidth + section * spot_on.getWidth(), height / 3 + textMove, mTextPaint);
            }
            section++;
        }
        if(cur_sections == section_title.size()-1){
            canvas.drawBitmap(thumb, width - spot_on.getWidth() - bitMapHeight/2 - thumb.getWidth() / 2,
                    height / 3 - bitMapHeight, buttonPaint);
        }else {
